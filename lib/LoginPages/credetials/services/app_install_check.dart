...
class AppInstallCheck {
  ...
  Future<bool> isModuleInstalled(String moduleName) async {
    try {
      ...
      try {
        ...
        /// If fallback succeeds, backend is reachable
        return true;
      } catch (_) {
        /// If fallback also fails → backend likely inaccessible
        return false;
      }
    }
  }

  /// Checks whether Attendance module is installed.
  ///
  /// Required for:
  ...
  Future<bool> checkRequiredModules() async {
    return await isModuleInstalled('hr_attendance');
  }

  /// Checks whether Leave Management module is installed.
  ///
  /// Required for:
  /// - Leave requests
  /// - Leave approvals
  /// - Leave balance tracking
  Future<bool> checkLeaveModule() async {
    return await isModuleInstalled('hr_holidays');
  }
}
