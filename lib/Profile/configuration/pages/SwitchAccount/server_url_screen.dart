...
import 'package:mobo_attendance/Profile/configuration/pages/SwitchAccount/switch_credentials_screen.dart';
import 'package:shared_preferences/shared_preferences.dart';

import '../../../../LoginPages/login/models/session_model.dart';
import '../../../../LoginPages/login/services/network_service.dart';
import '../../../../MainScreens/AppBars/pages/common_app_bar.dart';

/// First step of multi-account setup: enter Odoo server URL and select database.
///
/// Features:
/// - Auto-detects protocol (http/https) and tries both if not specified
/// - Debounced URL input → fetches available databases
/// - URL history with suggestions + preserved database/username
/// - Manual database input fallback if auto-detection fails
/// - Clean dark/light theme + background image
/// - Navigates to credentials screen on valid URL + database
...
class _ServerUrlScreenState extends State<ServerUrlScreen> {
  ...
  /// Loads saved URL history from SharedPreferences
  ...
  /// Saves current URL + database + username to history (most recent first, max 10)
  Future<void> saveUrlHistory({
    ...
    // Remove old entry for same URL/protocol
  ...
  /// Debounced handler for URL input changes
  ...
  /// Attempts to fetch list of databases from the given server URL.
  ///
  /// Tries both http/https if protocol not specified.
      ...
      for (String protocol in protocolsToTry) {
        try {
          ...
          if (dbList.isNotEmpty) {
            ...
            // Empty list but connection worked → manual input
            setState(() {
              _workingProtocol = protocol;
              _databases = [];
              _showManualDbInput = true;
          ...
          // Handle known Odoo error responses
  ...
  Widget build(BuildContext context) {
    ...
    return WillPopScope(
      onWillPop: () async {
        Navigator.push(
          context,
          MaterialPageRoute(builder: (context) => CommonAppBar()),
        );
        return false;
      },
      child: Scaffold(
  ...
  /// Manual database name input field (shown when auto-detection fails)
...
/// Custom URL input field with:
/// - Protocol selector (http/https)
/// - Autocomplete suggestions from history
/// - Loading indicator during database fetch
...
/// Dropdown to select from available Odoo databases after URL validation
