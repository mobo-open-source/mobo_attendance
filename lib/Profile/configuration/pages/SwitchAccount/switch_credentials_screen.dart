...
/// Second step of multi-account setup: enter username + password (or TOTP) to authenticate.
///
/// Flow:
/// 1. Validates form input
/// 2. Attempts login via `CompanySessionManager.loginAndSaveSession`
/// 3. On success:
///    - Saves account to secure storage
///    - Updates URL history
///    - Checks required modules (Attendance)
///    - Reloads app blocs and navigates to main screen
/// 4. On 2FA required → navigates to TOTP screen
/// 5. On failure → shows user-friendly error message
/// 6. Handles module-missing case with fallback cleanup + dialog
...
class _SwitchCredentialsScreenState extends State<SwitchCredentialsScreen> {
  ...
  /// Main login + account addition logic
  Future<void> _addAccount() async {
    ...
    try {
      ...
      if (context.mounted) {
        // Check if required modules (Attendance) are installed
  ...
  /// Shows dialog when required Attendance module is missing
  ...
  /// Maps raw login errors to user-friendly messages
  ...
  /// Reusable username/password input field
